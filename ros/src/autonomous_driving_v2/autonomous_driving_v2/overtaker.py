...
class Overtaker(Node):
    def __init__(self):
        ...
        self.speed = 0
        self.lc_time = 0
        self.left_lane_data = [210, 535, 330, 430, 262, 491]
        self.right_lane_data = [100, 425, 200, 300, 144, 377]
        self.left_driveway_factor = 0.25
        self.right_driveway_factor = 0.75
        ...
        self.sub_left = self.create_subscription(Range, 'tof_Front_Right', self.look_right, 1)
        ...
        self.pub_lane_data = self.create_publisher(Int64MultiArray, 'lane_data', 1)
        self.pub_driveway_factor = self.create_publisher(Float64, 'driveway_factor', 1)
    ...
    def is_road_blocked(self, msg):
        if self.current_phase == 0 and msg.range < 1.8 and self.left_lane_free and not self.blocked:
    ...
    def overtake(self):
        ...
        if self.current_phase == 1:
            self.change_lane(-20.0, 0.9)
            self.current_phase = 2
            print("---SET CURRENT PHASE TO 2---")
        # Phase 2 - Switch RoI and Hold Lane
        elif self.current_phase == 2:
            self.update_lane_data(self.left_lane_data, self.left_driveway_factor)
            msg_ld = Bool()
            msg_ld.data = False
        ...
        # Phase 4 - Hold Lane when box is gone
            ...
            msg_ld.data = True
            self.pub_block_lbs.publish(msg_ld)
            self.update_lane_data(self.right_lane_data, self.right_driveway_factor)
    ...
    def update_lane_data(self, dataset, df):
        out = Int64MultiArray()
        out.data = dataset
        self.pub_lane_data.publish(out)
        df_out = Float64()
        df_out.data = df
        self.pub_driveway_factor.publish(df_out)
    ...
    def look_left(self, msg):
        self.left_lane_free = 0.3 < msg.range

    def look_right(self, msg):
        self.right_lane_free = 0.3 < msg.range
